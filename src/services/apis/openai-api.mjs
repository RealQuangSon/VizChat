...
export async function generateAnswersWithChatgpt4vApi(port, question, session, apiKey, modelName) {
  ...
  if (session.image_url) {
    let finalImageUrl = session.image_url;
    
    // Kiểm tra: Nếu ảnh chưa có tiền tố 'data:' và không phải link 'http' -> Thêm header base64
    ...
    console.log('Image attached to request successfully')
  }
  // --- KẾT THÚC PHẦN SỬA ---
  ...
  console.log('Final prompt payload:', JSON.stringify(prompt, null, 2))
