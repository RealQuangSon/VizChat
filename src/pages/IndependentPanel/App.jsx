...
function App() {
  ...
  const saveRenameSession = async (session) => {
    ...
    if (!nextName || nextName === session.sessionName) return
    const updatedSession = {
      ...session,
      sessionName: nextName,
    }
    const updatedSessions = await updateSession(updatedSession)
    setSessions(updatedSessions)
    if (sessionId === updatedSession.sessionId) setCurrentSession(updatedSession)
  }
                ...
                >
                  {editingSessionId === session.sessionId ? (
                    <input
                      ...
                      onKeyDown={(e) => {
                        e.stopPropagation()
                        if (e.key === 'Enter') saveRenameSession(session)
                        if (e.key === 'Escape') cancelRenameSession()
                      }}
                    />
                  ) : (
                    <span className="session-name-text">{session.sessionName}</span>
                  )}
