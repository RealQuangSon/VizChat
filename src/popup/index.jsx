  ...
  if (config.clickIconAction === 'popup' || (window.innerWidth > 100 && window.innerHeight > 100)) {
    ...
    if (config.clickIconAction in menuConfig) {
      ...
      if (menuConfig[config.clickIconAction].genPrompt) {
        ...
        // 1. Chỉ chụp đúng phần màn hình đang hiển thị trước mặt (Viewport)
        const singleImage = await Browser.tabs.captureVisibleTab(null, { format: 'png' })
        
        // 2. Gửi ảnh vào background để lưu lại
        await Browser.runtime.sendMessage({ type: 'SAVE_IMAGE', imageUrl: singleImage })
        
        // 3. Kích hoạt khung chat
