...
import {
  buildAnnotatedQuestion,
  clearAnnotationOverlay,
  detectLikelyChartRect,
  extractAnnotationPayload,
  renderAnnotationOverlay,
  stripAnnotationPayload,
} from '../../content-script/annotation-overlay.mjs'
...
function ConversationCard(props) {
  ...
  const portMessageListener = (msg) => {
    ...
    if (msg.done) {
      ...
      setTimeout(() => {
        const latestItems = conversationItemDataRef.current
        const latestAnswer = [...latestItems]
          .reverse()
          .find((item) => item.type === 'answer')

        if (!latestAnswer?.content) return

        const payload = extractAnnotationPayload(latestAnswer.content)
        const cleanedAnswer = stripAnnotationPayload(latestAnswer.content)

        // Always hide internal annotation payload from chat text.
        if (cleanedAnswer !== latestAnswer.content) {
          setConversationItemData((old) => {
            const copy = [...old]
            const index = findLastIndex(copy, (v) => v.type === 'answer')
            if (index !== -1) {
              copy[index] = new ConversationItemData('answer', cleanedAnswer, true)
            }
            return copy
          })
        }

        // Draw only when annotate mode is explicitly enabled.
        if (currentSession.vizchatAnnotateMode && payload) {
          renderAnnotationOverlay(
            payload,
            currentSession.vizchatCapturedWidth,
            currentSession.vizchatCapturedHeight,
            currentSession.vizchatChartRect,
          )
        }
      }, 0)
    }
      ...
      <InputBox
        ...
        onAttachChart={async () => {
          ...
          if (!response || !response.ok || !response.imageUrl) {
            throw new Error('Unable to capture chart screenshot')
          }
        ...
        onSubmit={async (question, attachedImage, annotateOnBrowser, options = {}) => {
          if (options.clearOverlayOnly) {
            clearAnnotationOverlay()
            return
          }

          if (!annotateOnBrowser) clearAnnotationOverlay()
          const detectedChartRect = annotateOnBrowser ? detectLikelyChartRect() : null

          const questionForApi = annotateOnBrowser
            ? buildAnnotatedQuestion(
                question,
                attachedImage?.width,
          ...
          const newSession = {
            ...
            isRetry: false,
            image_url: attachedImage?.imageUrl || '',
            vizchatAnnotateMode: Boolean(annotateOnBrowser),
            vizchatCapturedWidth: attachedImage?.width || null,
            vizchatCapturedHeight: attachedImage?.height || null,
            vizchatChartRect: detectedChartRect,
          }
          setSession(newSession)
          try {
            await postMessage({ session: newSession })
