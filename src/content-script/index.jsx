...
async function mountComponent(siteConfig, userConfig) {
  console.log('mount component disabled')
  return
  const retry = 10
...
async function prepareForRightClickMenu() {
  ...
  Browser.runtime.onMessage.addListener(async (message) => {
      if (message.type === 'GET_PAGE_INFO') {
        // return page and viewport info to orchestrate full-page capture
        return {
      ...
      // console.log(await Browser.tabs.captureVisibleTab(message.tabId, {format: "png"}))

      let prompt = ''

      console.log('data in CREATE_CHAT:')
      console.log(data)
      console.log(message)
